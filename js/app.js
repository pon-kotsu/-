...
$(document).ready(function() {
  $.simpleWeather({
    ...
    success: function(weather) {
        //地名の表示
        html = '<p>'+"city :"+weather.city+'</p>';

        //気象状況を表示
        html += '<p class="currently">'+"weather　:"+weather.currently+'</pd>';

        //気温の表示
        html += '<p>'+"temperature　:"+weather.temp+'℃'+'</p>';
    ...
    if (sex) {
      if (10<age &&age<20) {
        if (T>25&&T<=35) {
         ...
         return "image/モンハン.jpg";
        }
        else if(T>35){
         return "image/man.jpg";
        }
        else{
...
//女性版画像の表示
      ...
      if (10<age&&age<=19){
        if (T>25&&T<=35){
           let photo = ["image/L１０代/L10代２５度 .jpg",
  ...
  document.body.appendChild(elm);
    let a = document.in.wether.value;
    console.log(a);
    if(a=="sunny"){ 
　 let text= document.createTextNode("It's such beautiful weather today!!");
　 document.body.appendChild(text);
    }
    else if(a=="cloudy"){
      let text= document.createTextNode("The sky looks threatening.");
　 document.body.appendChild(text);
    }
    else{
      let text= document.createTextNode("Terrible weather, so you should have a umbrella today.");
　 document.body.appendChild(text);
    }
    if(i>1){
      alert("これ以上ありませんリセットしてください");
      i=0;
    }else{
  i=i+1;
    }
